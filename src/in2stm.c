...
int main()
{
	...
	double t[10];
	long int m10,m11,m20,m21,m10_hr,m11_hr,m20_hr,m21_hr,nshift,nshift1,nshift2;
	// Parallelization
	double TK,TK2;
	/* Set pi value */
	pival=2.0*asin(1.0);
	...
	if (printmod){fprintf(fp_log,"... Generating initial prn file ... \n"); fflush(fp_log);}
	...
	/* Open init.t3c */
#if setup < 10
	fl1 = fopen("init_lab.t3c","rt");
#elif setup > 9
	fl1 = fopen("init_ls.t3c","rt");
#endif
	
	fprintf(fp_log,"Formation of Initial Conditions ...\n");
	...
	else
	{
		...
		fprintf(fp_log,"Set markers location \n");
	...
	fprintf(fp_log,"Read rocks properties \n");
	ffscanf1();
	while (sa[0]!='~')
	{
	...
	if(nonstab>0)
	{
		fprintf(fp_log,"Set Random nonstability on markers\n");
	...
	fprintf(fp_log,"Set boundary conditions\n");
	bondnum=1;
	ffscanf1();
	while (sa[0]!='~')
	{
		...
		if(sa[0] == 'M' || sa[1] == 'M') {n1=10; fprintf(fp_log,"Set new Cell markers location \n");}
		if(n1 == -1) {fprintf(fp_log,"Unknown Parameter <%s>",sa); exit(0);}
		/**/
		/**/
		/**/
		...
		/* fprintf(fp_log,"%d %ld %ld %ld %ld %e %e %e",n1,m10,m11,m20,m21,cnst,koef,ival);getchar(); */
		...
		/**/
		/**/
		/* Recalculate grid positions, since staggered needed for all different par's.. */
			...
			for (m2=m20;m2<=m21;m2++)
		{
			...
			switch(n1)
			{
				...
				/*
				fprintf(fp_log,"E  %e %e %e %e %e %e", ival,ival1,cnst,koef,ep[m1],ep[xnumx+m1]); getchar();
				*/
				break;
				/**/
				...
				/*
				fprintf(fp_log,"H  %e %e %e %e %e", ival,ival1,cnst,koef,ep[xnumx*2+m1]); getchar();
				*/
				break;
				/**/
				/**/
				...
				/*
				fprintf(fp_log,"L  %e %e %e %e %e", ival,ival1,cnst,koef,ep[xnumx*3+m1]); getchar();
				*/
				break;
				/**/
				/**/
				...
				//fprintf(fp_log,"X  %e %e %e %e %e %e \n", ival,ival1,cnst,koef,gx[m1-1],gx[m1]); //getchar();
				...
				//fprintf(fp_log,"Y  %e %e %e %e %e %e \n", ival,ival1,cnst,koef,gy[m2-1],gy[m2]); //getchar();
				...
				for (mm1=0;mm1<nshift;mm1++)
				{
					...
					for (mm2=0;mm2<nshift1;mm2++)
					{
						...
						if(cnst>0 && koef>0)
						{
							...
							/*
							fprintf(fp_log,"X  %ld %ld %ld %ld %e %e %e ", m1,m2,mm1,mm2,dx,dy,(double)(rand() % ((int)(cnst)*2+1) - (int)(cnst))/((double)(cnst))*(double)(dx*koef));getchar();
							*/
						}
						if(cnst>0 && koef1>0)
						{
							...
							/*
							fprintf(fp_log,"Y  %ld %ld %ld %ld %e %e %e ", m1,m2,mm1,mm2,dx,dy,(double)(rand() % ((int)(cnst)*2+1) - (int)(cnst))/((double)(cnst))*(double)(dy*koef1));getchar();
							*/
						}
						/* Add Y */
						y+=dy;
	...
	// For each line in init.t3c go through this loop
	fprintf(fp_log,"Set rocks bodies as markers types \n");
	ffscanf1();
	while (sa[0]!='~')
	{
		...
		/*
		fprintf(fp_log,"%d %d %d",yn,n1,n5); getchar();
		*/
		/**/
		...
		ffscanf1();
		/**/
		/* Add Markers in reactangle */
		if (yn==1)
		{
			fprintf(fp_log,"Set new markers location \n");
			dx=xstpx/(double)(mnumx);
			dy=ystpy/(double)(mnumy);
			/* Marker Types add */
				...
				for (m2=(int)(by[0]*(double)(ynumy-1));m2<(int)(by[3]*(double)(ynumy-1)+1.0);m2++)
			{
				...
				for (m4=0;m4<mnumx;m4++)
				{
					...
					{
						/* X,Y */
						markx[marknum]=(double)(x);
						marky[marknum]=(double)(y);
						markt[marknum]=n1;
						/* Random Nonstability Set on markers */
						if(nonstab>0)
						{
	...
	fprintf(fp_log,"Total amount of markers = %ld\n",marknum);
	/**/
	/**/
	/**/
	...
	if(nonstab<0)
	{
		fprintf(fp_log,"Set Sin nonstability on markers\n");
		for (mm1=0;mm1<marknum;mm1++)
			if(markt[mm1]<100)
		{
			if(nonstab==-1)
		...
		if(nonstab>0 && (mnumx>1 || mnumy>1))
		{
				...
				for (mm2=mm1;mm2<mm1+m3;mm2++)
			{
				...
				m2=rand() % (mnumy);
				m4=mm2;
				fprintf(fp_log,"%ld %ld %ld %ld %ld %ld %ld",mnumx,mnumy,m1,m2,mm1,mm2,m4);getchar();
				*/
	...
	fprintf(fp_log,"Read distribution of temperature \n");
	ffscanf1();
	while (sa[0]!='~')
	{
			...
			if(yn!=3)
			{
				...
				if(x>=x1 && x<=x2)
				{
					...
					if(y>=y1 && y<=y2)
					{
						...
						y=(y-y1)/(y2-y1);
						/*
						fprintf(fp_log,"%e %e %ld %ld",x,y,m1,m2);getchar();
						*/
						/**/
						if(yn==0)
						{
						...
						if(yn==4)
						{
							...
							/*
							fprintf(fp_log,"%ld %ld %ld  %e %e   %e %e   %e",m1,m2,m3,gx[m1],gy[m2],x,y,tk[m3]);getchar();
							*/
						}
						if(yn==5)
						{
						...
						/*
						fprintf(fp_log,"%e %e %ld %ld %ld %e",x,y,m1,m2,m3,tk[m3]);getchar();
						*/
					}
				}
			}
	...
	for (mm1=0;mm1<marknum;mm1++) 
		...
		if (markx[mm1]>0 && marky[mm1]>0 && markx[mm1]<xsize && marky[mm1]<ysize)
		{
			...
			allintertomp(markx[mm1],marky[mm1],m10,m20,&TK,&TK2);
			markk[mm1]=TK;
			//allintert(markx[mm1],marky[mm1]);
			//markk[mm1]=eps[2];
		}
	/**/
	/**/
	...
	for(n1=0;n1<2;n1++)
	{
		fprintf(fp_log,"2-nd Recalc nodes properties after markers types \n");
		ronurecalcomp();
		/**/
		/**/
		/**/
	...
	*/
	...
	fprintf(fp_log,"Saving information to output file \n");
	printmod=1;
	saver(0,0);
		
	// Also save to hdf5, so can visualize easily at start as well
	create_hdf5(_TRUE_, "setup001", 0);
	/**/
	/**/
	/**/
	fprintf(fp_log,"OK!\n");
